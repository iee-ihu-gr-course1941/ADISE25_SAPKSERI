...
### Περίληψη API
Το "The Dry Peaks API" (ADISE25_SAPKSERI) αποτελεί ένα project για το μάθημα Ανάπτυξη Διαδικτυακών  
...
**game.php:** Στο αρχείο game βρίσκονται οι συναρτήσεις για την δημιουργία,επανεκκίνηση,λήψη και ενημέρωση παιχνιδιου καθώς και οι κύριες συναρτήσεις του παιχνιδίου που προαναφέραμε καθώς και ορισμένοι getters.
  ...
  * **RoundShareCards($gameid):** Η συνάρτηση που καλούμε μετά του κάθε γύρου η οποία παίρνει κάρτες απο την τράπουλα που είναι αποθηκευμένη στην βάση και μοιράζει κάρτες στους παίχτες.
  * **changetoken($gameid):** Πολύ συναμαντική συνάρτηση για την ασφάλεια γιατι αλλάζει το token του χρήστη που έχει σειρά. Η ιδανική χρήση είναι μετά το throwcard() ώστε να ανατεθεί νέο token στον παίχτη που έχει σειρά.
...
```
* και τώρα θα χρειαστεί να διαγραφεί το παιχνίδι:
```json 
{
